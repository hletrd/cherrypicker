...
describe('buildSkipLinkUrl', () => {
  test('keeps query state and the fragment on a nested route under a document base', () => {
    const href = buildSkipLinkUrl(
      '/cherrypicker/cards/',
      '?issuer=shinhan&card=shinhan-test-card',
    );
    expect(href).toBe(
