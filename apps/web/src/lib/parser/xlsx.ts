...
function parseDateToISO(raw: unknown, errors?: ParseError[], lineIdx?: number): string {
  ...
  if (typeof raw === 'number') {
    ...
    // Parity with server-side XLSX parser in packages/parser/src/xlsx/index.ts.
...
function parseXLSXSheet(sheet: XLSX.WorkSheet, bank?: BankId, htmlBankHint?: BankId | null): ParseResult {
  ...
  // extends to all columns: date, merchant, category, installments,
  // memo, and amount (parity with server-side parser C89-01).
  ...
  for (let i = headerRowIdx + 1; i < rows.length; i++) {
    ...
    // Date column forward-fill (parity with server-side C47-01/C73-02)
    ...
    // Merchant column forward-fill (parity with server-side C47-01/C73-02)
    ...
    // Category column forward-fill (C52-06/C73-02, parity with server-side)
