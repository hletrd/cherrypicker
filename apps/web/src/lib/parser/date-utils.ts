...
export function parseDateStringToISO(raw: string): string {
  ...
  if (koreanFull) {
    ...
    // (C92-01). Parity with server-side packages/parser/src/date-utils.ts.
    return cleaned;
  }
 ...
 *  across all parsers to eliminate duplication (C97-03). Parity with
 *  server-side packages/parser/src/date-utils.ts. */
