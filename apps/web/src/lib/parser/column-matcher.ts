...
export function findColumn(headers: string[], exactName: string | undefined, pattern: RegExp): number {
  ...
  if (exactName) {
    ...
    for (let i = 0; i < headers.length; i++) {
      ...
      // Split combined headers on "/" or "|" and test each part (C43-01/C51-01)
  ...
  for (let i = 0; i < headers.length; i++) {
    ...
    // Split combined headers on "/" or "|" and test each part (C33-04/C51-01)
    if (normalized.includes('/') || normalized.includes('|')) {
...
// packages/parser/src/csv/column-matcher.ts.
export const DATE_COLUMN_PATTERN = /이용일|이용일자|거래일|거래일시|날짜|일시|결제일|승인일|승인일자|승인일시|매출일|작성일|접수일|발행일|사용일|사용일자|^date$|^trans(?:action)?[\s_-]?date$|^posted$|^billing$/i;
export const MERCHANT_COLUMN_PATTERN = /이용처|가맹점|가맹점명|이용가맹점|승인가맹점|거래처|매출처|사용처|결제처|상호|판매처|구매처|매장|취급처|이용내용|거래내용|^merchant$|^store$|^shop$|^vendor$|^description$|^item$|^name$|^payee$|^가게/i;
...
export const INSTALLMENTS_COLUMN_PATTERN = /할부|할부개월|할부기간|할부월|할부개월수|할부횟수|할부회수|^installments?$|^install$/i;
export const CATEGORY_COLUMN_PATTERN = /업종|카테고리|분류|업종분류|업종명|거래유형|결제유형|이용구분|구분|가맹점유형|^category$|^type$/i;
export const MEMO_COLUMN_PATTERN = /비고|적요|메모|내용|설명|참고|상세내역|^memo$|^note$|^remarks?$/i;
...
export const SUMMARY_ROW_PATTERN = /총\s*합\s*계(?![가-힣])(?=[\s,;]|$)|(?<![가-힣])합\s*계(?![가-힣])(?=[\s,;]|$)|(?<![가-힣])합\s*산(?![가-힣])|(?<![가-힣])소\s*계(?![가-힣])|(?<![가-힣])총\s*계(?![가-힣])|(?<![가-힣])누\s*계(?![가-힣])|(?<![가-힣])잔액(?![가-힣])|(?<![가-힣])당월(?![가-힣])|(?<![가-힣])명세(?![가-힣])|(?<![가-힣])이월\s*잔액(?![가-힣])|(?<![가-힣])전월\s*이월(?![가-힣])|(?<![가-힣])이월\s*금액(?![가-힣])|승인\s*합계|결제\s*합계|합\s*계\s*금액|총\s*(?:사용|이용)|\btotal\b|\bsum\b/i;
...
export const DATE_KEYWORDS: ReadonlySet<string> = new Set(['이용일', '이용일자', '거래일', '거래일시', '날짜', '일시', '결제일', '승인일', '승인일자', '승인일시', '매출일', '작성일', '접수일', '발행일', '사용일', '사용일자', 'date', 'posted', 'billing']);
export const MERCHANT_KEYWORDS: ReadonlySet<string> = new Set(['이용처', '가맹점', '가맹점명', '이용가맹점', '승인가맹점', '거래처', '매출처', '사용처', '결제처', '상호', '판매처', '구매처', '매장', '취급처', '이용내용', '거래내용', 'merchant', 'store', 'shop', 'description', 'vendor', 'item', 'name', 'payee']);
