...
// packages/parser/src/csv/column-matcher.ts.
export const DATE_COLUMN_PATTERN = /이용일|이용일자|거래일|거래일시|날짜|일시|결제일|승인일|승인일자|승인일시|매출일|작성일|접수일|발행일|사용일|사용일자|조회일|처리일|승인완료일|입금일|주문일|결제예정일|작성일시|승인시간|매입일|전표일|이용시간|취소일|정산일|환불일|반품일|교환일|^date$|^trans(?:action)?[\s_-]?date$|^purchase[\s_-]?date$|^order[\s_-]?date$|^posted$|^billing$|^book(?:ing)?[\s_-]?date$|^cancel[\s_-]?date$|^refund[\s_-]?date$|^settlement[\s_-]?date$/i;
export const MERCHANT_COLUMN_PATTERN = /이용처|가맹점|가맹점명|이용가맹점|승인가맹점|거래처|매출처|사용처|결제처|상호|판매처|구매처|매장|취급처|이용내용|거래내용|이용업소|승인점|매장명|이용매장|상호명|업체명|판매자|가맹점상호|거래내역|이용가맹점명|^merchant$|^store$|^shop$|^vendor$|^description$|^item$|^name$|^payee$|^seller$|^company$|^business$|^가게$|^recipient$|^outlet$/i;
export const AMOUNT_COLUMN_PATTERN = /이용금액|거래금액|금액|결제금액|승인금액|매출금액|이용액|취소금액|환불금액|입금액|결제액|청구금액|출금액|결제대금|승인취소금액|매입금액|실청구금액|실결제금액|결제예정금액|사용금액|환급금액|입금금액|실입금액|^amount$|^total$|^price$|^won$|^charge$|^payment$|^total[\s_-]?amount$|^paid$|^spent$|^cost$|^value$|^debit$|^credit$|^net$/i;
...
export const MEMO_COLUMN_PATTERN = /비고|적요|메모|내용|설명|참고|참고사항|상세내역|비고란|메모란|상세|비고내용|비고내역|메모내용|승인번호|카드번호|승인내역|비고사항|카드명|이용카드|^memo$|^note$|^remarks?$|^approval[\s_-]?(?:no|num|number)$/i;
...
export const SUMMARY_ROW_PATTERN = /총\s*합\s*계(?![가-힣])(?=[\s,;]|$)|(?<![가-힣])합\s*계(?![가-힣])(?=[\s,;]|$)|(?<![가-힣])합\s*산(?![가-힣])|(?<![가-힣])소\s*계(?![가-힣])|(?<![가-힣])총\s*계(?![가-힣])|(?<![가-힣])누\s*계(?![가-힣])|(?<![가-힣])잔액(?![가-힣])|(?<![가-힣])당월(?![가-힣])|(?<![가-힣])명세(?![가-힣])|(?<![가-힣])이월\s*잔액(?![가-힣])|(?<![가-힣])전월\s*이월(?![가-힣])|(?<![가-힣])이월\s*금액(?![가-힣])|(?<![가-힣])승인\s*합계(?![가-힣])|(?<![가-힣])결제\s*합계(?![가-힣])|(?<![가-힣])합\s*계\s*금액(?![가-힣])|(?<![가-힣])총\s*(?:사용|이용)(?![가-힣])|(?<![가-힣])사용\s*합계(?![가-힣])|(?<![가-힣])이용\s*합계(?![가-힣])|(?<![가-힣])총\s*결제\s*금액(?![가-힣])|(?<![가-힣])총\s*이용\s*금액(?![가-힣])|(?<![가-힣])총액(?![가-힣])|(?<![가-힣])당월\s*청구\s*금액(?![가-힣])|(?<![가-힣])이전\s*잔액(?![가-힣])|(?<![가-힣])결제\s*완료(?![가-힣])|(?<![가-힣])할인\s*합계(?![가-힣])|(?<![가-힣])포인트\s*사용(?![가-힣])|(?<![가-힣])포인트\s*적립(?![가-힣])|(?<![가-힣])할부\s*수수료\s*합계(?![가-힣])|(?<![가-힣])수수료\s*합계(?![가-힣])|(?<![가-힣])연체료\s*합계(?![가-힣])|(?<![가-힣])연체\s*이자(?![가-힣])|\btotal\b|\bsum\b|\bsubtotal\b/i;
...
export const HEADER_KEYWORDS: readonly string[] = [
  '이용일', '이용일자', '거래일', '거래일시', '날짜', '일시', '결제일', '승인일', '승인일자', '승인일시', '매출일', '작성일', '접수일', '발행일', '사용일', '사용일자', '조회일', '처리일', '승인완료일', '입금일', '주문일', '결제예정일', '작성일시', '승인시간', '매입일', '전표일', '이용시간', '취소일', '정산일', '환불일', '반품일', '교환일',
