...
export const AMOUNT_COLUMN_PATTERN = /이용금액|거래금액|금액|결제금액|승인금액|승인액|매출금액|이용액|취소금액|환불금액|입금액|결제액|청구금액|청구액|출금액|결제대금|승인취소금액|매입금액|실청구금액|실결제금액|실결제액|결제예정금액|사용금액|사용액|환급금액|입금금액|실입금액|이용대금|할인금액|포인트할인|할인전금액|할인후금액|^amount$|^total$|^price$|^won$|^charge$|^payment$|^total[\s_-]?amount$|^transaction[\s_-]?amount$|^payment[\s_-]?amount$|^billed[\s_-]?amount$|^paid$|^spent$|^cost$|^value$|^debit$|^credit$|^net$|^net[\s_-]?amount$|^gross$/i;
...
export const SUMMARY_ROW_PATTERN = /총\s*합\s*계(?![가-힣])(?=[\s,;]|$)|(?<![가-힣])합\s*계(?![가-힣])(?=[\s,;]|$)|(?<![가-힣])합\s*산(?![가-힣])|(?<![가-힣])소\s*계(?![가-힣])|(?<![가-힣])총\s*계(?![가-힣])|(?<![가-힣])누\s*계(?![가-힣])|(?<![가-힣])잔액(?![가-힣])|(?<![가-힣])당월(?![가-힣])|(?<![가-힣])명세(?![가-힣])|(?<![가-힣])이월\s*잔액(?![가-힣])|(?<![가-힣])전월\s*이월(?![가-힣])|(?<![가-힣])이월\s*금액(?![가-힣])|(?<![가-힣])승인\s*합계(?![가-힣])|(?<![가-힣])결제\s*합계(?![가-힣])|(?<![가-힣])합\s*계\s*금액(?![가-힣])|(?<![가-힣])총\s*(?:사용|이용)(?![가-힣])|(?<![가-힣])사용\s*합계(?![가-힣])|(?<![가-힣])이용\s*합계(?![가-힣])|(?<![가-힣])총\s*결제\s*금액(?![가-힣])|(?<![가-힣])총\s*이용\s*금액(?![가-힣])|(?<![가-힣])총액(?![가-힣])|(?<![가-힣])당월\s*청구\s*금액(?![가-힣])|(?<![가-힣])이전\s*잔액(?![가-힣])|(?<![가-힣])결제\s*완료(?![가-힣])|(?<![가-힣])할인\s*합계(?![가-힣])|(?<![가-힣])포인트\s*사용(?![가-힣])|(?<![가-힣])포인트\s*적립(?![가-힣])|(?<![가-힣])포인트\s*합계(?![가-힣])|(?<![가-힣])할부\s*수수료\s*합계(?![가-힣])|(?<![가-힣])수수료\s*합계(?![가-힣])|(?<![가-힣])연체료\s*합계(?![가-힣])|(?<![가-힣])연체\s*이자(?![가-힣])|(?<![가-힣])승인\s*취소\s*합계(?![가-힣])|(?<![가-힣])미결제\s*잔액(?![가-힣])|(?<![가-힣])카드\s*이용\s*합계(?![가-힣])|(?<![가-힣])카\s*드\s*이용\s*한도(?![가-힣])|\btotal\b|\bsum\b|\bsubtotal\b/i;
...
export const AMOUNT_KEYWORDS: ReadonlySet<string> = new Set(['이용금액', '거래금액', '금액', '결제금액', '승인금액', '승인액', '매출금액', '이용액', '취소금액', '환불금액', '입금액', '결제액', '청구금액', '청구액', '출금액', '결제대금', '승인취소금액', '매입금액', '실청구금액', '실결제금액', '실결제액', '결제예정금액', '사용금액', '사용액', '환급금액', '입금금액', '실입금액', '이용대금', '할인금액', '포인트할인', '할인전금액', '할인후금액', 'amount', 'total', 'price', 'won', 'charge', 'payment', 'paid', 'spent', 'cost', 'value', 'totalamount', 'transactionamount', 'paymentamount', 'billedamount', 'debit', 'credit', 'net', 'netamount', 'gross']);
